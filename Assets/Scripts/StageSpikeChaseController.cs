...
namespace DrawBody.Prototype
{
    ...
    public sealed class StageSpikeChaseController : StageEliminationChallengeController
    {
        ...
        private void RefreshStartSequence()
        {
            ...
            else main = LocalizationManager.T("spike_chase_run");
            ...
            if (monitorTitle != null) monitorTitle.text = LocalizationManager.T("spike_chase_monitor_title");
            if (monitorMain != null)
            {
                monitorMain.text = main;
                monitorMain.characterSize = main.Length > 4 ? 0.12f : 0.2f;
            }
            if (monitorSub != null) monitorSub.text = LocalizationManager.T(
                remaining > 0f ? "spike_chase_monitor_ready" : "spike_chase_monitor_goal");
        }
        ...
        private void BuildWall()
        {
            ...
            StagePillarSurvivalController.CreateRect(monitor.transform, new Vector2(7.2f, 2.15f), new Color(0.18f, 0.22f, 0.27f, 0.9f), -34);
            StagePillarSurvivalController.CreateRect(monitor.transform, new Vector2(6.7f, 1.72f), new Color(0.01f, 0.035f, 0.05f, 0.92f), -33);
            monitorTitle = StagePillarSurvivalController.CreateText(
                monitor.transform, "Title", new Vector3(0f, 0.57f, -0.02f), 40, 0.1f,
                new Color(0.56f, 0.9f, 1f, 1f), -30);
            monitorMain = StagePillarSurvivalController.CreateText(
                monitor.transform, "Main", new Vector3(0f, 0f, -0.03f), 72, 0.2f,
                new Color(0.18f, 1f, 0.68f, 1f), -29);
            monitorSub = StagePillarSurvivalController.CreateText(
                monitor.transform, "Sub", new Vector3(0f, -0.58f, -0.04f), 34, 0.075f,
                new Color(1f, 0.78f, 0.2f, 1f), -28);
            RefreshStartSequence();
        }
